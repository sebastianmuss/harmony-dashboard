...
export default function HomeContent() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-blue-50 to-slate-100 flex flex-col">
      ...
      {/* Header */}
      <header className="px-4 sm:px-8 py-4 flex items-center justify-between max-w-5xl mx-auto w-full gap-3">
        <div className="flex items-center gap-2 flex-shrink-0">
          <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center">
            <svg className="w-5 h-5 text-white" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 2C8 8 6 12 6 16a8 8 0 0 0 12 0C18 12 16 8 12 2z"/>
            </svg>
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
            <button
              onClick={() => setLang('en')}
              className={`px-2.5 py-1.5 text-xs font-semibold transition ${lang === 'en' ? 'bg-blue-600 text-white' : 'text-slate-500 hover:text-slate-700'}`}
            >EN</button>
            <button
              onClick={() => setLang('de')}
              className={`px-2.5 py-1.5 text-xs font-semibold transition ${lang === 'de' ? 'bg-blue-600 text-white' : 'text-slate-500 hover:text-slate-700'}`}
            >DE</button>
          </div>
          <a
            ...
            className="text-slate-400 hover:text-slate-600 transition p-1.5"
            aria-label="GitHub"
          >
